...
function bbp_redirect_to_field( $redirect_to = '' ) {
	...
	/* @richardmax - mega hack start ---- */
	if(strpos($_SERVER["REQUEST_URI"], 'nav=0') || strpos($_SERVER["REQUEST_URI"], '?/')){
		$redirect_to = $redirect_to . '?/'; // whooping hack - the nav=o kills the appended #4974947 post_id of the url. thus we now look for ? in the url on this page template of bbpress (loop-single-topic.php) -  ie. http://jsv1.ge0.uk/forums/topic/dam-condimentum-vulputate/?/#post-15100 instead of http://jsv1.ge0.uk/forums/topic/dam-condimentum-vulputate/?nav=0/#post-15100 (THIS BREAKS OBVS)
	}
